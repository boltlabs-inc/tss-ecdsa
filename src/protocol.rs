...
///
/// # 🔒 Message handling
/// The calling application is responsible for receiving, sending and routing all messages generated by participants of a sub-protocol session.
...
/// 2. Passing all messages for the session received from other participants
/// by calling [`Participant::process_single_message()`].
/// 3. Routing all messages generated by
/// [`Participant::process_single_message()`] to the correct recipient.
///
/// [`Message`]s contain a `to: ParticipantIdentifier` field which specifies the recipient of a message. The calling application is responsible for maintaining a mapping from [`ParticipantIdentifier`]s to some network metadata so messages can be routed to the correct `Participant`.
///
