...
use crate::{
    ...
    keyrefresh::keyshare::{KeyUpdatePrivate, KeyUpdatePublic},
    utils::CurvePoint,
...
/// Output type from key refresh, including all parties' *updates* to be applied to public key shares,
/// this party's *update* to be applied to the private key share.
#[derive(Debug, Clone)]
pub struct Output {
    public_updates: Vec<KeyUpdatePublic>,
    private_update: KeyUpdatePrivate,
}
...
impl Output {
    ...
    pub fn public_key(&self) -> Result<VerifyingKey> {
        ...
        VerifyingKey::from_encoded_point(&public_key_point.into()).map_err(|_| {
            error!("Keyrefresh output does not produce a valid public key.");
            InternalError::InternalInvariantFailed
        })
    }

    /// Get the individual shares of the public key.
    pub fn public_key_shares(&self) -> &[KeyUpdatePublic] {
        &self.public_updates
    }

    pub(crate) fn private_key_share(&self) -> &KeyUpdatePrivate {
        &self.private_update
    }
    ...
    pub(crate) fn private_pid(&self) -> Result<ParticipantIdentifier> {
        let expected_public_share = self.private_update.public_share()?;
        match self
            .public_updates
            .iter()
    ...
    pub fn from_parts(
        public_key_shares: Vec<KeyUpdatePublic>,
        private_key_share: KeyUpdatePrivate,
    ) -> Result<Self> {
        let pids = public_key_shares
            .iter()
            .map(KeyUpdatePublic::participant)
            .collect::<HashSet<_>>();
        if pids.len() != public_key_shares.len() {
            error!("Tried to create a keyrefresh output using a set of public material from non-unique participants");
        ...
        {
            error!("Tried to create a keyrefresh output using a private share with no corresponding public share");
            Err(CallerError::BadInput)?
        }

        Ok(Self {
            public_updates: public_key_shares,
            private_update: private_key_share,
        })
    }
    ...
    pub fn into_parts(self) -> (Vec<KeyUpdatePublic>, KeyUpdatePrivate) {
        (self.public_updates, self.private_update)
    }
}
...
mod tests {
    ...
    impl Output {
        /// Simulate the valid output of a keyrefresh run with the given
        /// participants.
        ///
        ...
        pub(crate) fn simulate(
            ...
            let (mut private_key_shares, public_key_shares): (Vec<_>, Vec<_>) = pids
                ...
                .map(|&pid| {
                    ...
                    let secret = KeyUpdatePrivate::random(rng);
                    let public = secret.public_share().unwrap();
                    (secret, KeyUpdatePublic::new(pid, public))
                })
                .unzip();
        ...
        /// Simulate a consistent, valid output of a keyrefresh run with the given
        /// participants.
        ///
        ...
        pub(crate) fn simulate_set(
            ...
            let (private_key_shares, public_key_shares): (Vec<_>, Vec<_>) = configs
                ...
                .map(|config| {
                    ...
                    let secret = KeyUpdatePrivate::random(rng);
                    let public = secret.public_share().unwrap();
                    (secret, KeyUpdatePublic::new(config.id(), public))
                })
                .unzip();
            ...
            private_key_shares
                ...
                .map(|private_key_share| {
                    Self::from_parts(public_key_shares.clone(), private_key_share).unwrap()
                })
                .collect()
        }
    }
    ...
    fn private_field_must_correspond_to_a_public() {
        ...
        let bad_private_key_share = KeyUpdatePrivate::random(rng);

        assert!(Output::from_parts(output.public_updates, bad_private_key_share).is_err())
    }
