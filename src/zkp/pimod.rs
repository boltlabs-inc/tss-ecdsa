...
use crate::{
    curve_point::CurveTrait, errors::*, utils::*, zkp::{Proof, ProofContext}
};
...
pub(crate) struct PiModProof<C: CurveTrait> {
...
impl<C: CurveTrait> Proof<C> for PiModProof<C> {
...
impl<C: CurveTrait> PiModProof<C> {
    fn fill_transcript(
        transcript: &mut Transcript,
        context: &impl ProofContext,
        input: &CommonInput,
...
mod tests {
    ...
    use crate::curve_point::{k256_order, CurvePoint};
    ...
    type TestFn = fn(PiModProof::<CurvePoint>, CommonInput, &mut StdRng) -> Result<()>;

    fn with_random_pimod_proof(
        rng: &mut StdRng,
        mut test_code: impl FnMut(PiModProof::<CurvePoint>, CommonInput, &mut StdRng) -> Result<()>,
    ) -> Result<()> {
        ...
        let proof_result = PiModProof::<CurvePoint>::prove(input, secret, &(), &mut transcript(), rng);
